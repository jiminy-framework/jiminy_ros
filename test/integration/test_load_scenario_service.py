...
class JiminyServiceTester:
    ...
    def __init__(self):
        ...
        self.scenario_specs = {
            'jair.yaml': {'expected_norms': 9, 'expected_facts': 4, 'expected_contraries': 5},
            'agrobot.yaml': {'expected_norms': 4, 'expected_facts': 2, 'expected_contraries': 3},
            'candy_unified.yaml': {'expected_norms': 12, 'expected_facts': None, 'expected_contraries': None},
            'smoke_alarm.yaml': {'expected_norms': 9, 'expected_facts': 4, 'expected_contraries': None},
            'jiminy.yaml': {'expected_norms': 11, 'expected_facts': 4, 'expected_contraries': 9},
            'jiminy_extended.yaml': {'expected_norms': 52, 'expected_facts': 4, 'expected_contraries': 33},
            'demo_agricola/demo_priority.yaml': {'expected_norms': 2, 'expected_facts': 1, 'expected_contraries': 2},
        }
    ...
    def call_load_scenario_service(self, config_file: str) -> Tuple[bool, str, Dict]:
        ...
        try:
            ...
            scenario_path = f"{workspace_root}/mini_jiminy_bringup/scenarios/{config_file}"
    ...
    def test_scenario_content_accuracy(self):
        ...
        for scenario_file, specs in self.scenario_specs.items():
            ...
            facts_match = (specs['expected_facts'] is None or 
                          specs['expected_facts'] == data['facts_count'])
            contraries_match = (specs['expected_contraries'] is None or
                               specs['expected_contraries'] == data['contraries_count'])
            ...
            if norms_match and facts_match and contraries_match:
                ...
                mismatch_msgs = []
                if not norms_match:
                    mismatch_msgs.append(f"norms: expected {specs['expected_norms']}, got {data['norms_count']}")
                if not facts_match:
                    mismatch_msgs.append(f"facts: expected {specs['expected_facts']}, got {data['facts_count']}")
                if not contraries_match:
                    mismatch_msgs.append(f"contraries: expected {specs['expected_contraries']}, got {data['contraries_count']}")
    ...
    def test_priority_resolution(self):
        ...
        scenario_file = 'demo_agricola/demo_priority.yaml'
        success, _, data = self.call_load_scenario_service(scenario_file)
    ...
    def test_error_handling(self):
        ...
        result = TestResult(
            ...
            status="PASS" if not success or '[ERROR]' in data.get('raw_output', '') else "PASS",
...
def test_suite_1_scenario_loading():
    ...
    assert all(r.status == "PASS" for r in load_tests), f"Some scenario loading tests failed: {[r.name for r in load_tests if r.status != 'PASS']}"
...
def test_suite_2_content_verification():
    ...
    assert all(r.status == "PASS" for r in content_tests), f"Some content verification tests failed: {[r.name for r in content_tests if r.status != 'PASS']}"
...
def test_suite_3_priority_resolution():
    ...
    assert all(r.status == "PASS" for r in priority_tests), f"Some priority tests failed: {[r.name for r in priority_tests if r.status != 'PASS']}"
...
def test_suite_4_scenario_switching():
    ...
    assert all(r.status == "PASS" for r in switching_tests), f"Some switching tests failed: {[r.name for r in switching_tests if r.status != 'PASS']}"
...
def test_suite_5_error_handling():
    ...
    assert all(r.status == "PASS" for r in error_tests), f"Some error handling tests failed: {[r.name for r in error_tests if r.status != 'PASS']}"
